...
## ⚡ 어려웠던 점 & 해결 과정
- **예약 로직**  
  -  날짜 기반 + 시퀀스 조합 방식으로 수정하면서 MyBatis `<selectKey>`와 Oracle 시퀀스 사용법을 새로 익혔습니다.
  -  계산로직을 짤때 어떻게하면 더 잘짤수있을까 계속 고민하며 짰던 것 같습니다.
  -  거의 막바지에 갑자기 결제실패가 되었는데도 예약됐다는 페이지로 넘어가는 에러가 떠서 유효성 검사를 한번 더 뜯어 보고 재 검토 하였습니다.

- **검색 & 페이징 처리**  
  - JSP에서 파라미터 전달이 제대로 되지 않아 검색 조건이 유지되지 않는 문제가 발생했습니다.  
  - 이를 해결하기 위해 JSTL `<c:set>`과 jQuery를 활용해 파라미터를 hidden 값으로 유지하고, 페이징 클릭 시에도 쿼리스트링을 함께 넘기도록 수정했습니다.  

- **비회원 예약 취소**  
  - 세션을 사용해 이메일 검증을 하려 했지만, 초기에는 세션이 제대로 갱신되지 않아 인증이 풀리는 문제가 있었습니다.  
  - 세션 확인 코드를 단순화하고, 파라미터와 함께 검증하도록 로직을 보완해 해결했습니다.  

- **GitHub 협업**  
  - 브랜치 충돌이 자주 발생해 작업 속도가 느려졌습니다.  
  - 기능별 브랜치를 세분화하고, Pull Request 기반으로 코드 리뷰 후 병합하는 프로세스를 도입해 문제를 줄였습니다.
