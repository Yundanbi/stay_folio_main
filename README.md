...
<details>
  <summary><b>2) 검색하지 않았을 때 기본 인원 처리 문제</b></summary>
...
<details>
  <summary><b>3) 예약번호 중복 방지 설계</b></summary>

**직면한 문제**  
- 단순 UUID는 사람이 읽기 어렵고, 날짜 기반 가독성 + 중복 방지가 필요.

**해결 과정**  
- `YYYYMMDD-시퀀스` 형태로 설계 (예: `20250811-000176`)  
- MyBatis `<selectKey>` + Oracle 시퀀스 조합으로 INSERT 이전에 PK 생성

**결과 및 학습점**  
- 중복 없는 가독성 높은 예약번호 확보  
- MyBatis 키 생성 흐름과 DB 시퀀스 동작을 실무 감각으로 이해
</details>
...
**직면한 문제**  
- 세션 갱신 타이밍 문제로 인증이 풀려 취소가 실패하는 경우 발생.

**해결 과정**  
- 세션 의존 로직을 단순화하고, 요청 파라미터(이메일)와 세션을 **병행 검증**  
- 검증 실패 시 명확한 에러 메시지와 재인증 플로우 제공

**결과 및 학습점**  
- 본인 확인의 신뢰도 상승, 예외 흐름 명확화  
- 인증/인가 로직은 “단순 + 명시적”이 유지보수에 유리함을 체감
</details>
