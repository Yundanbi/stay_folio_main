...
| 기능 | 화면 |
|------|------|
| 숙소</br> 검색</br> 결과 |![검색 쿼리 mp4](https://github.com/user-attachments/assets/370199b8-0ec9-4d7a-8c29-6b0360bdc8e3)|
...
- **예약 로직**  
  - 예약번호를 단순 UUID가 아닌 **날짜 + 시퀀스 조합 방식**으로 구현하며 MyBatis `<selectKey>`와 Oracle 시퀀스 사용법을 익혔습니다.  
  - 요금 계산 로직은 더 효율적인 구조를 고민하며 여러 번 개선했습니다.  
  - 결제 실패 시에도 예약 완료 페이지로 넘어가는 오류가 발생했으나, **유효성 검증 로직을 재점검**하여 문제를 해결했습니다.  

- **검색 & 페이징 처리**  
  - JSP에서 파라미터 전달 문제로 검색 조건이 유지되지 않았습니다.  
  - 이를 해결하기 위해 **JSTL `<c:set>`과 jQuery hidden 값**을 활용하여, 페이징 클릭 시에도 조건이 유지되도록 수정했습니다.  

- **비회원 예약 취소**  
  - 초기에는 세션 갱신 문제로 이메일 인증이 풀리는 오류가 있었습니다.  
  - 검증 로직을 단순화하고 **세션 + 파라미터 병행 검증 방식**으로 개선했습니다.  

- **GitHub 협업**  
  - 브랜치 충돌이 잦아 작업 지연이 발생했습니다.  
  - 기능별 브랜치를 세분화하고, **Pull Request 기반 코드 리뷰 → 병합** 프로세스를 도입하여 협업 효율을 높였습니다.  
